...
const adapters: Record<string, MarketSourceAdapter> = {
  ...
  investorlift: {
    ...
    searchUrlHint: 'Paste authorized InvestorLift deal or search URLs. DealFlowIQ will import up to the approved hourly limit.',
    urlExamples: ['https://app.investorlift.com/...'],
    importNotes: ['Uses the same live provider policy as other approved sources.', 'If InvestorLift returns an app/login shell, DealFlowIQ creates a review-required URL listing instead of silently failing.'],
    listingIdPatterns: [/dealId["'\s:]+([A-Za-z0-9_-]{5,})/i, /propertyId["'\s:]+([A-Za-z0-9_-]{5,})/i, /(?:deals?|properties?|listing)\/([A-Za-z0-9_-]{5,})/i],
    priceKeys: ['askingPrice', 'listPrice', 'price', 'offerPrice'],
    rentKeys: ['rent', 'monthlyRent', 'marketRent'],
    brokerKeys: ['seller', 'dispositionManager', 'agentName', 'brokerName'],
  },
  csv: {
    type: 'csv',
    label: 'CSV Feed',
