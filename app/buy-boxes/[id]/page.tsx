    ...
    <AppShell organizationName={workspace.organization?.name} userEmail={workspace.user.email} accountType={workspace.access.accountType} features={workspace.access.features} subscriptionStatus={workspace.access.status} planName={workspace.access.plan?.name} trialEndsAt={workspace.access.trialEndsAt} isPlatformAdmin={workspace.access.isPlatformAdmin}>
      <div className="space-y-6">
        ...
        <section className="grid gap-6 lg:grid-cols-4">
          ...
          <div className="rounded-3xl border border-white/10 bg-white/[0.03] p-5"><div className="text-xs text-slate-500">Opportunities</div><div className="mt-1 text-2xl font-black text-emerald-300">{row.last_opportunities_count || matches.filter((m) => Number(m.deal_score || 0) >= Number(row.min_deal_score || 80)).length}</div></div>
        </section>
          ...
          <div className="space-y-6">
            <div className="rounded-3xl border border-white/10 bg-white/[0.03] p-6">
              ...
              <form action={updateBuyBoxAction} className="mt-5 grid gap-4">
                ...
                <div className="grid gap-4 sm:grid-cols-3"><Field label="Max price" name="max_price" type="number" defaultValue={row.max_price} /><Field label="Min score" name="min_deal_score" type="number" defaultValue={row.min_deal_score || 80} /><Field label="Min rent confidence" name="min_rent_confidence" type="number" defaultValue={row.min_rent_confidence || 65} /></div>
