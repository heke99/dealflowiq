...
export async function generateImportPreviewAction(formData: FormData) {
  ...
  try {
    ...
    const urls = (batch as any).import_mode === 'search_url'
    ...
    if (!urls.length) throw new Error('No eligible listing URLs were found on the source page.')
    ...
    let inserted = 0
    let failed = 0
    for (const url of urls.slice(0, Math.min(remaining, 10))) {
      try {
        ...
        const [{ data: duplicate }, { data: ignored }] = await Promise.all([
          ...
          supabase.from('market_ignored_listings').select('*').eq('organization_id', workspace.organization.id).or(`source_url.eq.${normalized.source_url || url},and(normalized_address.eq.${normalizedAddress},zip_code.eq.${normalized.zip_code || ''})`).maybeSingle(),
        ])
        ...
        await supabase.from('market_import_preview_items').insert({
          ...
          source_url: normalized.source_url || url,
    ...
    await auditImportEvent(supabase, { organizationId: workspace.organization.id, userId: workspace.user.id, batchId, eventType: 'preview_generated', message: `Preview generated with ${inserted} listing(s).`, metadata: { found: urls.length, inserted, failed, sourceType } })
  } catch (error) {
