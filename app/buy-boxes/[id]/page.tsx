...
function Field({ label, name, placeholder, type = 'text', defaultValue }: { label: string; name: string; placeholder?: string; type?: string; defaultValue?: unknown }) {
  return <label className="block"><span className="text-sm font-medium text-slate-300">{label}</span><input name={name} type={type} defaultValue={String(defaultValue ?? '')} placeholder={placeholder} className="mt-2 w-full rounded-xl border border-white/10 bg-slate-900/80 px-4 py-3 text-slate-100 outline-none placeholder:text-slate-600 focus:border-white/30" /></label>
}
    ...
    <AppShell organizationName={workspace.organization?.name} userEmail={workspace.user.email} accountType={workspace.access.accountType} features={workspace.access.features} subscriptionStatus={workspace.access.status} planName={workspace.access.plan?.name} trialEndsAt={workspace.access.trialEndsAt} isPlatformAdmin={workspace.access.isPlatformAdmin}>
      <div className="space-y-6">
        <section className="rounded-3xl border border-white/10 bg-gradient-to-br from-purple-500/15 via-slate-950 to-black p-6 sm:p-8">
          ...
          <div className="mt-5 flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
            <div>
              ...
              <h1 className="mt-2 text-4xl font-black tracking-tight">{rowString(row.name)}</h1>
              <p className="mt-3 max-w-3xl text-slate-300">{rowString(row.description) || 'Automated market criteria for finding and ranking deals.'}</p>
            </div>
            <div className="flex flex-wrap gap-3">
              <form action={runBuyBoxNowAction}><input type="hidden" name="buy_box_id" value={String(row.id)} /><button className="rounded-xl bg-white px-5 py-3 text-sm font-semibold text-slate-950">Run now</button></form>
        ...
        <section className="grid gap-6 lg:grid-cols-4">
          ...
          <div className="rounded-3xl border border-white/10 bg-white/[0.03] p-5"><div className="text-xs text-slate-500">Matched</div><div className="mt-1 text-2xl font-black">{rowNumber(row.last_results_count) || matches.length || 0}</div></div>
          <div className="rounded-3xl border border-white/10 bg-white/[0.03] p-5"><div className="text-xs text-slate-500">Opportunities</div><div className="mt-1 text-2xl font-black text-emerald-300">{rowNumber(row.last_opportunities_count) || matches.filter((m) => Number(m.deal_score || 0) >= Number(row.min_deal_score || 70)).length}</div></div>
        </section>
        ...
        <section className="grid gap-6 xl:grid-cols-[0.9fr_1.1fr]">
          <div className="space-y-6">
            <div className="rounded-3xl border border-white/10 bg-white/[0.03] p-6">
              ...
              <form action={updateBuyBoxAction} className="mt-5 grid gap-4">
                <input type="hidden" name="buy_box_id" value={String(row.id)} />
                <Field label="Name" name="name" defaultValue={row.name} />
                <textarea name="description" rows={2} defaultValue={String(row.description || '')} className="w-full rounded-xl border border-white/10 bg-slate-900/80 px-4 py-3 text-slate-100 outline-none focus:border-white/30" />
                ...
                <div className="grid gap-4 sm:grid-cols-2"><label className="block"><span className="text-sm font-medium text-slate-300">Strategy</span><select name="strategy" defaultValue={String(row.strategy || 'buy_hold')} className="mt-2 w-full rounded-xl border border-white/10 bg-slate-900/80 px-4 py-3 text-slate-100"><option value="section8">Section 8</option><option value="buy_hold">Buy & Hold</option><option value="brrrr">BRRRR</option><option value="flip">Fix & Flip</option><option value="wholesale">Wholesale</option><option value="commercial">Commercial</option></select></label><label className="block"><span className="text-sm font-medium text-slate-300">Schedule</span><select name="schedule_frequency" defaultValue={String(row.schedule_frequency || 'daily')} className="mt-2 w-full rounded-xl border border-white/10 bg-slate-900/80 px-4 py-3 text-slate-100"><option value="manual">Manual only</option><option value="hourly">Hourly</option><option value="twice_daily">Twice daily</option><option value="daily">Daily</option><option value="weekly">Weekly</option></select></label></div>
              ...
              <form action={archiveBuyBoxAction} className="mt-4"><input type="hidden" name="buy_box_id" value={String(row.id)} /><button className="rounded-xl border border-red-500/30 px-5 py-3 text-sm font-semibold text-red-100 hover:bg-red-500/10">Archive Buy Box</button></form>
            </div>

            <div className="rounded-3xl border border-white/10 bg-white/[0.03] p-6">
              <h2 className="text-xl font-bold">Sources</h2>
              <div className="mt-4 space-y-3">{(sourcesResult.data || []).map((source: Row) => <div key={String(source.id)} className="rounded-2xl border border-white/10 bg-slate-950/40 p-4"><div className="font-semibold">{rowString(source.source_name)}</div><div className="mt-1 text-xs text-slate-500">{rowString(source.source_type)} · {rowString(source.schedule_frequency)} · {source.auto_import_enabled ? 'Auto' : 'Manual'}</div></div>)}{!(sourcesResult.data || []).length ? <p className="text-sm text-slate-500">No sources attached yet.</p> : null}</div>
            </div>
          </div>
          ...
          <div className="space-y-6">
            ...
            <div className="rounded-3xl border border-white/10 bg-white/[0.03] p-6">
              ...
              <div className="mt-5 space-y-3">{(jobsResult.data || []).map((job: Row) => <div key={String(job.id)} className="rounded-2xl border border-white/10 bg-slate-950/40 p-4"><div className="flex items-center justify-between"><div className="text-sm font-semibold">{rowString(job.job_type)?.replaceAll('_', ' ')}</div><span className="rounded-full border border-white/10 px-3 py-1 text-xs text-slate-300">{rowString(job.status)}</span></div><div className="mt-2 text-xs text-slate-500">Created {rowNumber(job.items_created) || 0} · Updated {rowNumber(job.items_updated) || 0} · Failed {rowNumber(job.items_failed) || 0}</div>{job.error_message ? <div className="mt-2 text-xs text-red-200">{String(job.error_message)}</div> : null}</div>)}</div>
            </div>
          </div>
        </section>
      </div>
