  ...
  return (
    <article className="overflow-hidden rounded-3xl border border-white/10 bg-white/[0.035] p-4">
      ...
      <div className="mt-4 flex items-start justify-between gap-3">
        <div>
          <Link href={`/market/${listing.id}`} className="text-lg font-bold hover:underline">{String(listing.title || listing.address || 'Saved listing')}</Link>
        ...
        <div className="flex flex-col items-end gap-2">
          ...
          <span className="rounded-full border border-white/10 bg-white/10 px-3 py-1 text-xs font-semibold text-slate-200">{dealStatusLabel(dealStatus)}</span>
        </div>
      </div>
      ...
      {listing.review_reason ? <p className="mt-3 text-xs text-slate-400">Review: {String(listing.review_reason)}</p> : null}
      <div className="mt-4 grid gap-2 sm:grid-cols-2">
        <form action={updateMarketListingReviewStatusAction}><input type="hidden" name="listing_id" value={String(listing.id)} /><input type="hidden" name="deal_status" value="ready" /><input type="hidden" name="review_reason" value="Moved from watchlist to opportunity after manual review." /><input type="hidden" name="return_to" value="/saved-deals" /><button className="w-full rounded-xl bg-emerald-300 px-3 py-2 text-sm font-semibold text-slate-950">Move to opportunity</button></form>
        <form action={convertListingToDealAction}><input type="hidden" name="listing_id" value={String(listing.id)} /><button className="w-full rounded-xl bg-white px-3 py-2 text-sm font-semibold text-slate-950">Analyze</button></form>
        <form action={saveOpportunityAction}><input type="hidden" name="listing_id" value={String(listing.id)} /><input type="hidden" name="status" value="contacted" /><button className="w-full rounded-xl border border-white/10 px-3 py-2 text-sm font-semibold text-slate-100">Contacted</button></form>
        <form action={saveOpportunityAction}><input type="hidden" name="listing_id" value={String(listing.id)} /><input type="hidden" name="status" value="passed" /><button className="w-full rounded-xl border border-white/10 px-3 py-2 text-sm font-semibold text-slate-100">Pass</button></form>
      </div>
    </article>
  )
}
  ...
  return (
    <AppShell organizationName={workspace.organization?.name} userEmail={workspace.user.email} accountType={workspace.access.accountType} features={workspace.access.features} subscriptionStatus={workspace.access.status} planName={workspace.access.plan?.name} trialEndsAt={workspace.access.trialEndsAt} isPlatformAdmin={workspace.access.isPlatformAdmin}>
      <div className="space-y-6">
        ...
        {(watchRows || []).length ? <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">{(watchRows || []).map((item: Row) => item.market_listings ? <SavedCard key={String(item.id)} item={item} score={scoreMap.get(String(item.listing_id)) || null} buyerMatchCount={buyerMatchCount.get(String(item.listing_id)) || 0} /> : null)}</div> : <div className="rounded-3xl border border-dashed border-white/15 bg-white/[0.03] p-10 text-center"><h2 className="text-xl font-bold">No saved deals yet</h2><p className="mt-2 text-slate-400">Save listings from Market or Opportunities to build your personal deal pipeline.</p><Link href="/opportunities" className="mt-5 inline-flex rounded-xl bg-white px-5 py-3 text-sm font-semibold text-slate-950">Open Opportunities</Link></div>}
      </div>
    </AppShell>
  )
}
