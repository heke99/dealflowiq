...
const SUBSCRIPTION_STATUSES = new Set(['trialing', 'active', 'past_due', 'canceled', 'expired', 'comped', 'manually_granted'])
...
export async function syncOrganizationSubscriptionAction(formData: FormData) {
  ...
  const { error } = await supabase.from('organization_subscriptions').upsert({
    ...
    trial_source: isTrial ? 'admin_override' : 'admin_override',
