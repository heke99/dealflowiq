...
function Badge({ children }: { children: React.ReactNode }) {
  return <span className="rounded-full border border-white/10 bg-white/[0.03] px-2.5 py-1 text-[11px] font-semibold uppercase tracking-wide text-slate-400">{children}</span>
}
...
function Field({ label, name, placeholder, defaultValue }: { label: string; name: string; placeholder?: string; defaultValue?: string }) {
  return (
    <label className="block">
      ...
      <input name={name} defaultValue={defaultValue || ''} placeholder={placeholder} className="mt-2 w-full rounded-xl border border-white/10 bg-slate-900/80 px-3.5 py-2.5 text-sm text-slate-100 outline-none placeholder:text-slate-600 focus:border-white/30" />
    </label>
  )
}
  ...
  return (
    ...
    >
      <div className="space-y-5">
        <section className="rounded-3xl border border-white/10 bg-gradient-to-br from-slate-900 via-slate-950 to-black p-5 sm:p-6">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            <div>
              <div className="text-sm font-medium uppercase tracking-wide text-slate-500">Market</div>
              <h1 className="mt-2 text-2xl font-black tracking-tight sm:text-3xl">Source imports & deal discovery</h1>
              ...
              <div className="mt-4 flex flex-wrap gap-2">
                ...
                <Badge>InvestorLift 40/hour</Badge>
                <Badge>Market first</Badge>
              </div>
            </div>
            <Link href="/market?tab=sources" className="rounded-xl border border-white/10 px-4 py-2.5 text-center text-sm font-semibold text-slate-100 transition hover:bg-white/10">Open full import center</Link>
          </div>
        </section>
        ...
        <section className="grid gap-5 xl:grid-cols-[0.72fr_1.28fr]">
          <div className="rounded-3xl border border-white/10 bg-white/[0.03] p-4 sm:p-5">
            <div className="flex items-start justify-between gap-4">
              <div>
                <h2 className="text-lg font-bold">Import URL now</h2>
                <p className="mt-1.5 text-sm leading-6 text-slate-400">
                  Paste a direct listing or search URL. Successful imports redirect straight to the listing.
                </p>
              </div>
            ...
            {canImportSources ? (
              <form action={importMarketUrlAction} className="mt-5 grid gap-4">
                <label className="block">
                  <span className="text-sm font-medium text-slate-300">Source</span>
                  <select name="source_type" defaultValue="manual_url" className="mt-2 w-full rounded-xl border border-white/10 bg-slate-900/80 px-3.5 py-2.5 text-sm text-slate-100 outline-none focus:border-white/30">
                ...
                <label className="block">
                  ...
                  <select name="visibility" defaultValue="private" className="mt-2 w-full rounded-xl border border-white/10 bg-slate-900/80 px-3.5 py-2.5 text-sm text-slate-100 outline-none focus:border-white/30">
                ...
                <SubmitButton pendingText="Importing and redirecting..." className="rounded-xl bg-white px-4 py-2.5 text-sm font-semibold text-slate-950 hover:bg-slate-200">Import and open listing</SubmitButton>
              </form>
            ) : (
          ...
          <div className="rounded-3xl border border-white/10 bg-white/[0.03] p-4 sm:p-5">
            ...
            <div className="mt-4 space-y-2.5">
              {(jobs || []).length ? (jobs || []).map((job: Row) => {
                ...
                const previewRows = Array.isArray(summary.previewRows) ? summary.previewRows : Array.isArray(summary.rows) ? summary.rows : []
                const listingIds = Array.isArray(job.normalized_listing_ids) ? job.normalized_listing_ids : []
                return (
                  <div key={job.id} className="rounded-2xl border border-white/10 bg-slate-950/40 p-3">
                    ...
                    {previewRows.length ? (
                      <div className="mt-3 rounded-xl border border-white/10 bg-black/20 p-2.5">
                        ...
                        <div className="mt-2 space-y-2">
                          {previewRows.slice(0, 5).map((row: Row, index: number) => (
                            <div key={index} className="rounded-lg border border-white/10 bg-white/[0.03] p-2 text-[11px]">
                    ...
                    <div className="mt-3 flex flex-wrap gap-2">
                      {listingIds.length ? <Link href={`/market?tab=all&import_job_id=${job.id}`} className="rounded-lg bg-white px-3 py-1.5 text-xs font-semibold text-slate-950 hover:bg-slate-200">View imported listings in Market</Link> : null}
                      {Number(summary.topScore || 0) >= OPPORTUNITY_SCORE_THRESHOLD ? <Link href="/opportunities" className="rounded-lg border border-emerald-400/30 px-3 py-1.5 text-xs font-semibold text-emerald-100 hover:bg-emerald-400/10">View Opportunities</Link> : null}
                    </div>
